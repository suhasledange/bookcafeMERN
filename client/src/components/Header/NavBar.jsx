...
const NavBar = () => {
  ...
  return (
    <header
      className={`shadow-sm w-full duration-200 py-3 md:py-2 h-full justify-center gap-3 bg-white flex flex-col items-center z-40 sticky top-0 transition-transform `}
    >
      <Container className="z-40 flex justify-between h-full items-center md:py-0 relative">
        ...
        {/* Hamburger icon ends */}
          ...
          <div onClick={() => setMobileMenu(false)}>
            <h1 className="text-md md:text-xl tracking-wider font-bold text-gray-900 ">
              BookCafe
            </h1>
          </div>
        ...
        <div
          className={`md:hidden transform duration-200 ${
            mobileMenu ? "translate-x-0" : " -translate-x-full "
          } absolute top-[100%] mt-3 w-4/6 left-0 h-screen `}
        >
          <div className="h-screen z-40 bg-white flex flex-col pt-5 border-t px-4 items-start justify-start font-medium text-md tracking-wide text-gray-700 space-y-5">
            {links.map((link) => (
              <div key={link.id} className="group w-full">
                <div
                  onClick={() => setMobileMenu(false)}
                  className=" flex items-center cursor-pointer justify-start gap-2 text-gray-900  border-b w-full py-2"
                  href={link.href}
                >
                  {link.icon} {link.text}
                </div>
        ...
        <div
          ...
          } h-screen absolute top-[100%] left-0 bg-transparent -z-50`}
        ></div>
        ...
        <div className="hidden w-full md:flex items-center justify-between">
          <div className=" border-b-2 p-1 pb-[0.35rem] gap-2 md:flex flex-[0.8] items-center justify-center mx-auto relative">
            ...
            {inputSearch.trim() !== "" &&
              (loading ? (
                <div className="z-50 absolute w-full top-12 mt-1 rounded-md p-4 left-0 shadow-xl bg-white space-y-2">
                ...
                <div className="z-50 absolute w-full top-12 mt-1 rounded-md p-4 left-0 shadow-xl bg-white">
                  {searchResult?.map((res) => (
                    <div
                      key={res.$id}
                      onClick={clearSearch}
                      className="flex items-center justify-start gap-2 w-full text-sm text-gray-700 p-2 hover:bg-gray-100"
                      href={`/book/${res.$id}`}
                    >
                      <IoSearch className=" shrink-0 text-md" /> {res.bookName}{" "}
                      by {res.author}
                    </div>
                  ))}
                </div>
        ...
        <div className="flex items-center gap-2 text-black ml-0 md:ml-4">
          <IoSearch
            onClick={handleClick}
            className=" text-gray-600 text-lg cursor-pointer md:hidden"
          />
            ...
            {/* {!status ? (
              ...
              <div className="">
                <div
                  ...
                  className=" w-8 h-8 md:w-10 md:h-10 flex items-center justify-center cursor-pointer rounded-full"
                >
                  <Image
                    className="rounded-full"
                    src={photo ? `${photo}` : "/DefaultProfile.svg"}
                    alt="noimg"
                    width={500}
                    height={500}
                  />
                ...
                {profileMenu === true ? (
                  <div>
                    <div
                      ...
                      className="absolute top-0 left-0 bg-transparent w-full h-screen"
                    ></div>
      ...
      <div
        className={`${
          search ? "block" : "hidden"
        } md:hidden border-2 mx-auto rounded-md px-2 py-1 gap-2 flex w-[96%] items-center justify-center relative`}
      >
        <IoSearch className=" text-gray-600 text-lg" />
        <input
          ref={ref}
          ...
          (loading ? (
            ...
            <div className="z-50 absolute w-full top-12 mt-1 rounded-md p-4 left-0 bg-white shadow-xl">
              {searchResult?.map((res) => (
                ...
                //   href={`/book/${res.$id}`}
                >
                  <IoSearch className="text-md shrink-0" /> {res.bookName} by{" "}
                  {res.author}
                </div>
              ))}
            </div>
